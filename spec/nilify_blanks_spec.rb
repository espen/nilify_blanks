...
RSpec.describe NilifyBlanks do
  context "Model with nilify_blanks" do
    ...
    it "should recognize all non-null string, text, citext columns" do
      expect(Post.nilify_blanks_columns).to eq(['first_name', 'title', 'summary', 'body', 'slug', 'blog_id', 'custom_data'])
    end
    ...
    it "should convert all blanks to nils" do
      ...
      expect(@post.custom_data).to be_nil if array_supported?
    end
  ...
  context "Model with nilify_blanks :nullables_only => false" do
    ...
    it "should recognize all (even null) string, text, citext columns" do
      expect(PostWithNullables.nilify_blanks_columns).to eq(['first_name', 'last_name', 'title', 'summary', 'body', 'slug', 'blog_id', 'custom_data'])
    end
  end
  ...
  context "Model with nilify_blanks :except => [:first_name, :title, :blog_id]" do
    ...
    it "should recognize only summary, body, views and custom_data" do
      expect(PostExceptFirstNameAndTitle.nilify_blanks_columns).to eq(['summary', 'body', 'slug', 'custom_data'])
    end
    ...
    it "should convert summary and body blanks to nils" do
      ...
      expect(@post.body).to be_nil
      expect(@post.slug).to be_nil
      expect(@post.custom_data).to be_nil
    end
